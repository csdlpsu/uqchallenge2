...
'''
This loop is going to do a monte-carlo simulation at each of the 7 training 
points provided by Boeing. Essentially, an interval is created based on the 
...
GP (XFOIL + correction GP). The mean @ the training point is also plotted in 
dotted line. 
'''

for idx in range(len(true_x)):
    ...
    y_center_true = true_y[idx,:].numpy()   # (3,)
    ...
    for j in range(3):
        ...
        plt.axvline(y_center_true[j], color='black', linewidth=2, label="True")
